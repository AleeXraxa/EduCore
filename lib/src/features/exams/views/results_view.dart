            ...
            const Center(child: Padding(padding: EdgeInsets.all(48.0), child: CircularProgressIndicator()))
             ...
             Center(
               child: Padding(
                 ...
                 child: Column(
                   children: [
                     Icon(Icons.emoji_events_outlined, size: 64, color: cs.primary.withValues(alpha: 0.2)),
                     const SizedBox(height: 16),
                     const Text('No results compiled yet.', style: TextStyle(fontWeight: FontWeight.bold)),
                     const SizedBox(height: 8),
                     const Text('Ensure all marks are entered, then click "Generate Results".'),
                   ],
                 ),
               ),
             )
                       ...
                       return DataRow(cells: [
                         DataCell(
                           Container(
                             ...
                             decoration: BoxDecoration(
                               color: (r.rank != null && r.rank! <= 3) ? Colors.amber.withValues(alpha: 0.2) : Colors.transparent,
                               shape: BoxShape.circle,
                             ),
                             child: Text('${r.rank ?? '-'}', style: const TextStyle(fontWeight: FontWeight.w900)),
                           )
                         ),
                         DataCell(Text(r.studentName, style: const TextStyle(fontWeight: FontWeight.w800))),
                         DataCell(Text('${r.totalMaxMarks}')),
                         DataCell(Text('${r.totalObtained}')),
                         DataCell(Text('${r.percentage.toStringAsFixed(1)}%')),
                         DataCell(Text(r.grade, style: TextStyle(fontWeight: FontWeight.bold, color: cs.primary))),
                         DataCell(
                           Text(
                             r.status.toUpperCase(),
                             style: TextStyle(
                               color: r.status.toLowerCase() == 'pass' ? Colors.green : Colors.red,
