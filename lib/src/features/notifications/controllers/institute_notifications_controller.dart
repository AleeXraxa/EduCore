...
class InstituteNotificationsController extends BaseController {
    ...
    await runBusy(() async {
      try {
        final results = await Future.wait([
          _firestore.collection('academies').doc(_academyId).collection('students').get(),
          _firestore.collection('academies').doc(_academyId).collection('staff').get(),
        ]);

        _students = results[0].docs.map((d) => Student.fromMap(d.id, d.data() as Map<String, dynamic>)).toList();
        _staff = results[1].docs.map((d) => StaffMember.fromFirestore(d)).toList();
      } catch (e) {
  ...
  void _listenToLogs() {
    ...
    _logsSub = _firestore
        ...
        .snapshots()
        .listen((snap) {
      _messages = snap.docs.map((d) => WhatsAppMessage.fromFirestore(d)).toList();
      
      // Calculate stats
      final today = DateTime.now();
      _sentToday = _messages.where((m) => 
      ...
      _failedCount = _messages.where((m) => m.status == WhatsAppMessageStatus.failed).length;
  ...
  Future<void> connectWhatsApp() async {
    ...
    await runBusy(() async {
      try {
        ...
        _whatsappService?.connectToLiveUpdates(_academyId!, 
          onQrReceived: (qr) {
            _qrCode = qr;
            _whatsappStatus = 'qr_ready';
            notifyListeners();
  ...
  /// Send Single Message
  Future<bool> sendSingleMessage(BuildContext context, {
    ...
    final result = await runGuarded<bool>(() async {
      // 1. Send via Backend
      ...
      return success;
    }, context: context, loadingMessage: 'Sending Message...');
    
    return result ?? false;
  }

  /// Send Bulk Messages
  Future<void> sendBulkMessages(BuildContext context, {
    required List<Map<String, String>> recipients, // [{to, message, studentId, studentName}]
    required String broadcastType,
  }) async {
    if (_academyId == null) return;

    await runGuarded(() async {
      final List<Map<String, String>> payload = recipients.map((r) => {
        'to': r['to']!,
        'message': r['message']!,
      }).toList();

      // 1. Send via Backend
      final results = await _whatsappService?.sendBulk(
        academyId: _academyId!,
        messages: payload,
      ) ?? [];
      ...
      for (var i = 0; i < recipients.length; i++) {
        ...
        final res = results.firstWhere((element) => element['to'] == r['to'], orElse: () => {'success': false});
      ...
      await AppServices.instance.getAuditLogService.logAction(
        ...
        targetId: _academyId!,
        metadata: {
          'description': 'Bulk WhatsApp broadcast ($broadcastType) sent to ${recipients.length} recipients',
        }
      );

      return true;
    }, context: context, loadingMessage: 'Sending Bulk Messages...');
  }
